...
int tar_get_next_hdr(struct filespec *fs)
{
    ...
    fsclear(fs);
    while(1) {
	count = fs->io_func(&tarhead, 1, 512, fs->io_handle);
	...
	tarheadchk = tarhead;
	memcpy(tarheadchk.chksum, "        ", 8);
	for (tmpchksum = 0, p = (char *) (&tarheadchk), i = 512; i != 0; --i, ++p)
	    tmpchksum += 0xFF & *p;
	sprintf(tarheadchk.chksum, "%6.6o", tmpchksum);
	if (strcmp(tarhead.chksum, tarheadchk.chksum) != 0)
	    fprintf(stderr, "Tar checksum %s does not computed checksum %s\n", tarhead.chksum, tarheadchk.chksum);
	...
	if (*(tarhead.ftype) == 'L') {
		...
		count = fs->io_func(fs->filename + tcount, 1, bytestoread - tcount, fs->io_handle);
		if (count == 0) {
		...
		count = fs->io_func(junk, 1, blockpad - tcount, fs->io_handle);
		if (count == 0) {
		    fprintf(stderr, "Error\n");
		    return(0);
		}
	...
	if (*(tarhead.ftype) == 'K') {
	    ...
	    tcount = 0;
	    while (bytestoread - tcount > 0) {
		count = fs->io_func(fs->linktarget + tcount, 1, bytestoread - tcount, fs->io_handle);
		if (count == 0) {
		    fprintf(stderr, "Error\n");
		    return(0);
		}
	...
	if (*(tarhead.ftype) == '0' || *(tarhead.ftype) == 'S') {
		...
		while (s_isextended == 1) {
		    count = fs->io_func(&speh, 1, 512, fs->io_handle);
		    if (count < 512) {
			fprintf(stderr, "Error\n");
			return(0);
		    }
		...
		fs->ftype = '0';
		paxsparsehdrsz = 0;
		paxsparsehdrsz += c_getline(&paxsparsesegt, fs->io_func, fs->io_handle);
		...
		if ((paxsparsehdrsz % 512) > 0) {
		    paxsparsehdrsz += fs->io_func(junk, 1, 512 - paxsparsehdrsz % 512, fs->io_handle);
		}
...
int tar_write_next_hdr(struct filespec *fs) {
	...
	*(tarhead.ftype) = 'K';
	...
	fs->io_func(&tarhead, 1, 512, fs->io_handle);  // write out long file name header
	for (i = 0; i < strlen(fs->linktarget); i += 512) {
	    ...
	    fs->io_func(curblock, 1, 512, fs->io_handle); // write out long file name data
	}
    }
	...
	*(tarhead.ftype) = 'L';
	...
	for (tmpchksum = 0, p = (char *) (&tarhead), i = 512;
	    ...
	    tmpchksum += 0xFF & *p;
	sprintf(tarhead.chksum, "%6.6o", tmpchksum);
	fs->io_func(&tarhead, 1, 512, fs->io_handle);  // write out pax header

	for (i = 0; i < fs->xheaderlen; i += 512) {
	    memset(curblock, 0, 512);
	    memcpy(curblock, fs->xheader + i, fs->xheaderlen - i >= 512 ? 512 :
		(fs->xheaderlen - i));
	    fs->io_func(curblock, 1, 512, fs->io_handle);  // write out pax data
	}
	if (fs->ftype == 'g')
	    return(0);
    }
	...
	*(tarhead.ftype) = 'S';
	...
	tmpchksum += 0xFF & *p;
    sprintf(tarhead.chksum, "%6.6o", tmpchksum);
    fs->io_func(&tarhead, 1, 512, fs->io_handle);
   ...
   if (tarhead.u.sph.isextended == 1) {
	...
	for (i = 4; i < fs->n_sparsedata; i++) {
		...
		fs->io_func(&speh, 1, 512, fs->io_handle);
		memset(&speh, 0, sizeof(speh));
	    }
	}
	if ((i - 4) % 21 != 0) {
	...
	int sbufn;
	paxsparsehdrsz = 0;
	sbufn = sprintf(sbuf, "%d\n", (unsigned int) fs->n_sparsedata);
	paxsparsehdrsz += fs->io_func(sbuf, 1, sbufn, fs->io_handle);
	for (int i = 0; i < fs->n_sparsedata; i++) {
	    sbufn = sprintf(sbuf, "%llu\n", fs->sparsedata[i].offset);
	    paxsparsehdrsz += fs->io_func(sbuf, 1, sbufn, fs->io_handle); 
	...
	memset(curblock, '\0', 512);
	if ((paxsparsehdrsz % 512) > 0)
	    paxsparsehdrsz += fs->io_func(curblock, 1, 512 - paxsparsehdrsz % 512, fs->io_handle);
    }
    return(0);
}
...
int getpaxvarlist(char *paxdata, int paxlen, char **namelist) {
    char *nvp = paxdata;
    int nvplen;
    char *cname = NULL;
    int cnamelen = 0;
	...
	cnamelen = value - cname;
	value++;
	memcpyao((void **) namelist, cname, cnamelen, sz);
	sz += cnamelen;
...
int fsdup(struct filespec *tsf, struct filespec *sfs)
{
    ...
    tsf->pax = sfs->pax;
    tsf->io_func = sfs->io_func;
    tsf->io_handle = sfs->io_handle;
    return(0);
}
...
size_t tarsplit_write(void *buf, size_t sz, size_t count, struct tarsplit_file *tsf)
{
	...
	fsclear(tsf->tmp_fs);
    fs = tsf->tmp_fs;
    fs->io_func = tsf->c_fwrite;
    fs->io_handle = tsf->c_handle;
